...
class MedicalRecordForm:
    ...
    def create_entry_field(self, parent, section_title, field):
        ...
        if field == "Patient Name":
            # Sample list — replace with fnc.get_patient_names() or similar
    ...
    def submit_section(self, section_name):
        values = self.form_vars[section_name]
        empty = []
        for field, var in values.items():
            if isinstance(var, tk.Text):
                if not var.get("1.0", "end-1c").strip():
                    empty.append(field)
            else:
        ...
        if empty:
            messagebox.showwarning("Incomplete Form", f"Please fill in the following fields in {section_name}:\n• " + "\n• ".join(empty))
            return

        summary = "\n".join(
            f"{field}: {var.get('1.0', 'end-1c') if isinstance(var, tk.Text) else var.get()}"
            for field, var in values.items()
        )
        messagebox.showinfo(f"{section_name} Submitted", f"Successfully submitted:\n\n{summary}")
