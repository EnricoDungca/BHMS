...
from Front_End.PagesGUI import Inventory as InventoryPage
...
class InventoryManagementApp:
    ...
    def add_inventory(self):
        self.root.destroy()
        inventoryForm.main(self.user_id)
    ...
    def create_table(self, parent):
        ...
        tbl = tk.Frame(frame, bg="white")
        tbl.pack(fill=tk.BOTH, expand=True)
        headers = ["Item ID","Name","Category","Qty","Price","Date Added","Actions"]
        col_w = [100,250,250,250,250,250,200]
        hdr = tk.Frame(tbl, bg="#f5f5f5")
        hdr.pack(fill=tk.X)
        for i, h in enumerate(headers):
            tk.Label(hdr, text=h, font=self.table_font, bg="#f5f5f5",
                    width=col_w[i]//10, anchor="w").pack(side=tk.LEFT)

        body = tk.Frame(tbl, bg="white")
    ...
    def populate_table(self, parent, col_w, query=""):
        ...
        for r in items:
            if query and not any(query in str(field).lower() for field in r):
                continue
            item = {"ID":r[0],"name":r[2],"category":r[3],"quantity":r[4],
                    "price":f"₱{r[5]:,.2f}","date":r[1]}
            self.add_row(parent, item, col_w)

    def add_row(self, parent, item, col_w):
        row = tk.Frame(parent, bg="white")
        row.pack(fill=tk.X, pady=4)
        vals = [item["ID"], item["name"], item["category"], item["quantity"],
    ...
    def perform_search(self, e=None):
        ...
        if q == "search inventory items...": q = ""
        for w in self.list_frame.winfo_children(): w.destroy()
        self.populate_table(self.list_frame, [100,200,150,100,100,150,200], q)
